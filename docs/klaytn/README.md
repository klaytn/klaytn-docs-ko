...
코어 셀(CC)은 하나의 <strong>컨센서스 노드(CN)</strong>와 두 개의 <strong>프록시 노드(PN)</strong>로 이루어집니다. 합의 노드는 블록 생성 프로세서에 참여하고, 프록시 노드는 네트워크에 인터페이스를 제공합니다. PN은 트랜잭션 요청을 합의 노드로 전송하고 블록을 엔드포인트 노드로 전파합니다.
